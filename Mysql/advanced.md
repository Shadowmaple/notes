...
27. **可以通过修改系统变量`innodb_flush_log_at_trx_commit`的值，来改变执行事务时刷新磁盘记录的模式**：
     - 0：在事务提交时不立即向磁盘中同步`redo`日志，任务交给后台线程做。
        - 这样很明显会加快请求处理速度，但是如果事务提交后服务器挂了，后台线程没有及时将redo⽇志刷新到磁盘，那么该事务对⻚⾯的修改会丢失。
     - 1：事务提交时立即将`redo`日志同步到磁盘，可以保证事务的持久性，默认值。
     - 2：事务提交时将`redo`日志写到**操作系统的缓冲区**中，但并不需要保证将日志真正的刷新到磁盘。
        - 这种情况下如果数据库挂了，操作系统没挂的话，事务的持久性还是可以保证的，但是操作系统也挂了的话，那就不能保证持久性了。
28. 【重点】使用 redo 日志进行崩溃恢复：
     ...
     3. 使用哈希表，通过`space ID`和`page number`计算出散列值，遍历哈希表恢复。同时使用拉链法，若有多个散列值相同的redo⽇志，就使⽤链表连接起来。如此相比直接遍历redo日志，避免很多读取页面的随机I/O。
     4. 跳过已经刷新到磁盘的页面……
...
3. 事务id的生成和分配策略：
    ...
    - 当系统下一次重新启动时，会将上边提到的`Max Trx ID`属性加载到内存中，将该值加上256之后赋值给我们前边提到的全局变量（因为在上次关机时该全局变量的值可能大于`Max Trx ID`属性值，比如上次的值为257，那么重新启动后加载到内存再加上256的值就是512，虽然258-511的数都没用过，但**如此保证了id是唯一的，并且是递增的**）
...
6. 向某个表中插入一条记录时，实际上需要向聚簇索引和所有的二级索引都插入一条记录。不过记录undo日志时，只需要考虑向聚簇索引插入记录时的情况就好了，因为其实聚簇索引记录和二级索引记录是一一对应的，在回滚插入操作时，只需要知道这条记录的主键信息，然后根据主键信息做对应的删除操作。（**版本链只在聚簇索引**，那不回表的select怎么办？）
    - **二级索引的 `DB_TRX_ID` 复用**：二级索引记录虽然没有完整版本链，但会存储 **最后修改该索引记录的事务 ID（`DB_TRX_ID`）**。
    - **结合聚簇索引的版本链做可见性判断**：
        - 当查询二级索引时，InnoDB 会先根据二级索引记录的 `DB_TRX_ID` 做初步筛选，排除明显不可见的版本（如未提交事务修改的记录）。
        - 对于可能可见的记录，通过其包含的 **主键** 到聚簇索引中查询完整的版本链，最终确定该记录对当前事务是否可见。
...
11. IS、IX锁是表级锁，它们仅仅为了在之后加表级别的S锁和X锁时可以快速判断表中的记录是否被上锁，以避免用遍历的方式来查看表中有没有上锁的记录。**IS锁和IX锁是兼容的，IX锁和IX锁是兼容的**
