...
3.  服务器收到后，会发送SYN+ACK报文作为应答，也指定了自身的初始序列号y，并把客户端的序列号x+1作为ack值，表示已经收到了客户端的SYN，此时服务器处于SYN_RCVD状态；
4.  客户端收到后，发送ACK报文，携带序列号x+1，将服务端的序列号y+1作为ack值，表示已经收到了服务端的SYN报文，状态变为ESTABLISHED
5.  服务端收到ACK报文后，状态变为ESTABLISHED，此时双方建立了连接。
...
所以可以说三次握手是可以传输数据的，但一般在第三次客户端ACK报文段传输可能会比较多，因此这时客户端已进入ESTABLISHED状态。但若是承载数据，服务端就要花费额外的内存去存储，而由于SYN段比较容易遭受到攻击（洪泛攻击），那么这时内存就很容易被消耗完。
...
初始序列号不是固定的，而是动态生成的，不同主机、同一主机在不同时刻都不相同。
...
-   TCP报文段是以IP数据报的形式传输的，IP数据报拥有**TTL字段**和跳数限制字段，这两个字段限制了IP数据报的有效生存时间，即MSL。
-   RFC中定义为2分钟，但实际是可以修改的，Linux系统中可以修改参数`net.ipv4.tcp_fin_timeout`值为30秒--5分钟，大多为30秒。

作用：保证双方正常关闭连接

按照规则，TIME_WAIT在等待2MSL时，其端口号不能被其它连接使用。但大多数系统都允许进程先对其进行绑定，但在2MSL有效期内，仍不能进行连接建立等操作。

若MSL时间很短，即来不及保证服务器重传的FIN抵达，客户端就关闭了：

1.  Socket结构回收了，操作系统找不到目标的socket和端口号，会返回一个RST报文；
2.  重开了一个进程，恰巧是同样的端口号，即同样的Socket四元组：
    1.  向服务器建立SYN请求，服务器在等待ACK却收到了SYN，会回复RST，导致连接建立失败；
    2.  收到了服务器重传的FIN，无法识别，回复RST；



#### CLOSE_WAIT

CLOSE_WAIT状态需要Socket等待上层应用程序调用`close()`关闭后才能进入下一个状态。
...
-   客户端**机器挂了**：
    -   **没有重启**：服务端发送的数据会被丢弃，收不到ACK，就会每隔一段时间进行重传，直到次数到达一个阈值，断开连接，并向对方发送一个RST报文，这个过程会持续几分钟到十来分钟。（不会检查到不可达吗？？）
    -   **迅速重启**：
...
>   RFC那边不同意保活这个功能（理由看《卷一》），这个功能存在争议性，但最后大部分TCP的实现还是提供了。（2333）
...
-   收到SYN报文，但不存在目的端口，即主机上其进程不存在；
-   需要取消一个已有的TCP连接；
-   收到一个不存在的socket连接的报文（连接队列中不存在该socket连接）。



#### TIME_WAIT过多

TIME_WAIT直到2MSL后才会消失，所以TIME_WAIT过多是因为有太多TCP短连接打开，建立和使用该TCP连接的时间远小于TIME_WAIT失效的时间。

解决办法：

1.  复用TCP连接，更好利用TCP连接资源，如HTTP1.1使用长连接；
2.  适当更改MSL数值；
3.  设置内核参数，快速销毁TIME_WAIT。

>   [参考](https://mp.weixin.qq.com/s/Jt2ss3SsE_iGJp5oW8QDSw)
...
Q10：若三次握手第一次SYN滞留网络了，等待重传建立连接后才到达服务端，这时面对同样的Socket四元组，怎么处理？
